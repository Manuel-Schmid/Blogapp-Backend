...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_create_category_too_long_fields(import_query: Callable, client_query: Callable) -> None:
    category_input = {
        'categoryInput': {
            'name': 'e' * 201,
        }
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_update_category(create_categories: Callable, import_query: Callable, client_query: Callable) -> None:
    create_categories()
